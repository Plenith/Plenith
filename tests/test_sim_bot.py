...
class TestSnapshotConsistency:
    def test_same_hour_same_fleet(self, sim_bot):
        t1 = dt.datetime(2026, 5, 11, 14, 0, 0, tzinfo=dt.timezone.utc)
        t2 = dt.datetime(2026, 5, 11, 14, 30, 0, tzinfo=dt.timezone.utc)
    ...
    def test_outside_work_hours_no_one_online(self, sim_bot):
        ...
        snap = sim_bot._snapshot(dt.datetime(2026, 5, 11, 3, 0, 0, tzinfo=dt.timezone.utc))
...
class TestRenderers:
    def test_who_format(self, sim_bot):
        t = dt.datetime(2026, 5, 11, 14, 0, 0, tzinfo=dt.timezone.utc)
    ...
    def test_uptime_format(self, sim_bot):
        out = sim_bot.uptime_output(now=dt.datetime(2026, 5, 11, 14, 0, 0, tzinfo=dt.timezone.utc))
        assert "up" in out
        assert "load average" in out
        assert "users" in out

    def test_last_includes_reboot(self, sim_bot):
        out = sim_bot.last_output(now=dt.datetime(2026, 5, 11, 14, 0, 0, tzinfo=dt.timezone.utc))
        assert "reboot" in out
        assert "wtmp begins" in out

    def test_w_has_header(self, sim_bot):
        out = sim_bot.w_output(now=dt.datetime(2026, 5, 11, 14, 0, 0, tzinfo=dt.timezone.utc))
        assert "load average" in out
